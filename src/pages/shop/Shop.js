        ...
        <div className='w-10/12 mx-auto lg:px-10 my-5'>
         <h2 className='text-xl uppercase font-semibold my-5'>Your total recipe: {orders.length}</h2>
           {orders.map(order=><div key={order._id}>
                ...
                <div className='grid grid-cols-3 m-5 place-items-center px-10 py-4 mx-auto'>
                    <div className='lg:w-full'>
                        <div className='w-16 lg:w-32 h-16 lg:h-32 rounded-full flex flex-col justify-center items-center place-items-center bg-violet-200'>
                            <Link to={`/edit/${order._id}`}>
                                <GrDocumentUpdate className='text-3xl lg:text-7xl font-bold text-blue-800'></GrDocumentUpdate>
                            </Link>
                        </div>
                    </div>
                    <div className='w-full flex flex-col justify-center items-center place-items-center'>
                        <div>
                            <img src={order.strMealThumb} alt="" className='w-16 lg:w-24 rounded'/>
                        </div>
                        <h2 className='text-sm text-white'>{order.strMeal.slice(0,10)}</h2>
                        <h2 className='text-sm text-white'>{order.email.slice(0,10)}<span title={order.email}>...</span></h2>
                    </div>
                   <div className='w-full flex justify-end'>
                        <div className='w-16 lg:w-32 h-16 lg:h-32 rounded-full bg-red-200 flex flex-col justify-center items-center place-items-center'>
                                <button onClick={()=>handleDelete(order._id)} className='mx-2'>
                                    <RiDeleteBin6Line className='text-3xl lg:text-7xl font-semibold text-red-500'>
                                    </RiDeleteBin6Line>
                                </button>
                        </div>
                   </div>
